...
namespace WWC.MonoBehaviours
{
    ...
    public class PlasmaWeapon_Mono : MonoBehaviourPun, IPointStartHookHandler, IPointEndHookHandler, IBattleStartHookHandler
    {
        ...
        private void AttackHandler()
        {
            ...
            if (ready)
            {
                if (inputSync.shootWasPressed && (int)gunAmmo.GetFieldValue("currentAmmo") > 0 && canShoot && (bool) data.playerVel.GetFieldValue("simulated"))
                {
        ...
        private class VelocityBooster : MonoBehaviourPun
        {
            ...
            private void RPCA_UpdateSpeed(float speedMult)
            {
                if (!updated)
                {
                    ...
                    WillsWackyCards.instance.DebugLog($"[{WillsWackyCards.ModInitials}][PlasmaMono] Increasing bullet velocity by {string.Format("{0:F0}", (speedMult - 1f) * 100)}%.");
