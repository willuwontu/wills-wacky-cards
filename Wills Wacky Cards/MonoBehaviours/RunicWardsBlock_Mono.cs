...
namespace WWC.Patches
{
    ...
    class ProjectileHitPatchRPCA_DoHit
    {
        ...
        //[HarmonyPatch("RPCA_DoHit")]
        //[HarmonyPrefix]
        //private static void AngleBlock(ProjectileHit __instance, Vector2 hitPoint, Vector2 hitNormal, Vector2 vel, int viewID, int colliderID, ref bool wasBlocked)
        //{
        //    HitInfo hitInfo = new HitInfo();
        //    hitInfo.point = hitPoint;
        //    hitInfo.normal = hitNormal;
        ...
        //        // If some condition isn't met, we should leave now.
        //        if (true)
        //        {
        //            return;
        //        }

        //        // Get the direction we're pointing, cast to vector2 for sanity.
        //        Vector2 handDir = (Vector2) data.hand.transform.forward.normalized;

        //        // Get the direction we're getting hit from, cast to vector2 for sanity.
        //        Vector2 hitDir = ((Vector2) ((Vector3) hitPoint - data.player.transform.position)).normalized;

        //        // Get a vector of the angular section we want to check in. Note, it's halved since we want to center on our hand.
        //        const float angle = 90f;
        //        Vector2 direction = (new Vector2(Mathf.Cos(angle / 2f * Mathf.Deg2Rad), Mathf.Sin(angle / 2f * Mathf.Deg2Rad))).normalized;

        //        // By adding/subtracting the angle, we get the boundary we're deflecting in.
        //        Vector2 a = handDir + direction;
        //        Vector2 b = handDir - direction;

        //        // If the angle between the hit point and both boundaries is less than the angle we're checking in, it's between those vectors.
        //        if (Vector2.Angle(a, hitDir) < angle && Vector2.Angle(b, hitDir) < angle)
        //        {
        //            wasBlocked = true;
        //        }
        //        else
        //        {
        //            // If we were blocking, then we're not anymore.
        //            if (wasBlocked)
        //            {
        //                wasBlocked = false;
        //            }
